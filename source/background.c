...
int background_functions(
  ...
  /* majoron-neutrino -- start */
  if (pba->has_maj == _TRUE_){
    // set variables
    double Temp_maj                       = pvecback_B[pba->index_bi_Temp_maj];
    pvecback[pba->index_bg_Temp_maj]      = Temp_maj;
    double Temp_nu_int                    = pvecback_B[pba->index_bi_Temp_nu_int];
    pvecback[pba->index_bg_Temp_nu_int]   = Temp_nu_int;
    double mu_maj                         = pvecback_B[pba->index_bi_mu_maj];
    pvecback[pba->index_bg_mu_maj]        = mu_maj;
    double mu_nu_int                      = pvecback_B[pba->index_bi_mu_nu_int];
    pvecback[pba->index_bg_mu_nu_int]     = mu_nu_int;

    // the class_test is needed to avoid segmentation fault in the integration step...
    class_test(pow(_E_,Temp_maj) <=0.,pba->error_message, "T_maj = %e instead of strictly positive", pow(_E_,Temp_maj));
    class_test(pow(_E_,Temp_nu_int) <=0.,pba->error_message, "T_nu_int = %e instead of strictly positive", pow(_E_,Temp_nu_int));

    // the neutrinos
    pvecback[pba->index_bg_rho_nu_int]  = -_eV4_to_Mpc2_*pba->N_nu_maj_int*pba->gnu*3./pow(_PI_,2)*pow(pow(_E_,pvecback_B[pba->index_bi_Temp_nu_int]),4.)* 
                                          polylog(4,-pow(_E_, pvecback_B[pba->index_bi_mu_nu_int] / pow(_E_,pvecback_B[pba->index_bi_Temp_nu_int])));
    double rho_nu   = pvecback[pba->index_bg_rho_nu_int];
    ...
    //the majoron
    pvecback[pba->index_bg_rho_maj] = _eV4_to_Mpc2_ * pba->gphi * pow(pow(10,pba->mass_maj),2.) * pow(_E_,pvecback_B[pba->index_bi_Temp_maj]) * 
                                      exp(pvecback_B[pba->index_bi_mu_maj] / pow(_E_,pvecback_B[pba->index_bi_Temp_maj])) * 1./2. * 1./pow(_PI_,2.) * 
                                      (pow(10,pba->mass_maj) * bessel(1,pow(10,pba->mass_maj) / pow(_E_,pvecback_B[pba->index_bi_Temp_maj])) + 
                                      3. * pow(_E_,pvecback_B[pba->index_bi_Temp_maj]) * bessel(2,pow(10,pba->mass_maj) / pow(_E_,pvecback_B[pba->index_bi_Temp_maj])));
    pvecback[pba->index_bg_p_maj]   = _eV4_to_Mpc2_ * pba->gphi* pow(pow(10,pba->mass_maj),2.) * pow(_E_,pvecback_B[pba->index_bi_Temp_maj]) * 
                                      exp(pvecback_B[pba->index_bi_mu_maj] / pow(_E_,pvecback_B[pba->index_bi_Temp_maj])) * 1./2. * 1./pow(_PI_,2.) * 
                                      pow(_E_,pvecback_B[pba->index_bi_Temp_maj]) * bessel(2,pow(10,pba->mass_maj) / pow(_E_,pvecback_B[pba->index_bi_Temp_maj]));

    rho_tot   += pvecback[pba->index_bg_rho_maj];
    p_tot     += pvecback[pba->index_bg_p_maj];
    rho_r     += 3. * pvecback[pba->index_bg_p_maj];
    rho_m     +=  pvecback[pba->index_bg_rho_maj] - 3. * pvecback[pba->index_bg_p_maj];
    if (pow(_E_,pvecback_B[pba->index_bi_Temp_maj]) / pow(10,pba->mass_maj) > 10.){
      dp_dloga  += -(4./3.) * pvecback[pba->index_bg_rho_maj];
    }
  }
  /* majoron-neutrino -- end */
...
int background_init(
  ...
  if (pba->background_verbose > 0) {
    ...
    if ((pba->N_ncdm > 0) || (pba->Omega0_idr != 0.) || (pba->N_nu_maj_int > 0))  {

      /* contribution of ultra-relativistic species _ur to N_eff */
      Neff = pba->Omega0_ur/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;

      /* majoron-neutrino -- start */
      Neff += pba->Omega0_nu_int/7.*8./pow(4./11.,4./3.)/pba->Omega0_g;
      printf("Neff from int. nu: %e\n", pba->Omega0_nu_int/7.*8./pow(4./11.,4./3.)/pba->Omega0_g);
      /* majoron-neutrino -- end */
...
int background_indices(
  ...
  pba->has_fld = _FALSE_;
  /* majoron-neutirno -- start*/
  pba->has_ur =  _TRUE_; //_FALSE_;
  /* majoron-neutirno -- end*/
  pba->has_idr = _FALSE_;
  ...
  /* majoron-neutrino -- start */
  if (pba->N_nu_maj_int > 0){
    printf("Majoron Evolution with n=%d interacting Neutrinos.\n", abs(pba->N_nu_maj_int));
    pba->has_maj = _TRUE_;
    pba->has_ur = _TRUE_;
  }
  /* majoron-neutrino -- end */
  ...
  /* majoron-neutrino -- start */
  // define index for class output in table
  class_define_index(pba->index_bg_Temp_maj,pba->has_maj,index_bg,1);
  class_define_index(pba->index_bg_Temp_nu_int,pba->has_maj,index_bg,1);
  class_define_index(pba->index_bg_mu_maj,pba->has_maj,index_bg,1);
  class_define_index(pba->index_bg_mu_nu_int,pba->has_maj,index_bg,1);
  class_define_index(pba->index_bg_rho_maj,pba->has_maj,index_bg,1);
  class_define_index(pba->index_bg_p_maj,pba->has_maj,index_bg,1);
  class_define_index(pba->index_bg_rho_nu_int,pba->has_maj,index_bg,1);
  /* majoron-neutrino -- end */
  ...
  class_define_index(pba->index_bi_Temp_maj,pba->has_maj,index_bi,1);
  class_define_index(pba->index_bi_Temp_nu_int,pba->has_maj,index_bi,1);
  class_define_index(pba->index_bi_mu_maj,pba->has_maj,index_bi,1);
  class_define_index(pba->index_bi_mu_nu_int,pba->has_maj,index_bi,1);
  /* majoron-neutrino -- end */
...
int background_solve(
  ...
  while (pvecback_integration[pba->index_bi_a] < pba->a_today) {
    ...
    if (pba->has_maj == _TRUE_){
      double aux_fine_step_size   = pba->fine_steps_maj;
      double aux_step_size        = ppr->back_integration_stepsize;
      double aux_limit            = 10.;

      if ( fabs( pow(_E_,pvecback[pba->index_bg_Temp_nu_int]) / pow(10,pba->mass_maj) ) < aux_limit && fabs( pow(_E_,pvecback[pba->index_bg_Temp_nu_int]) / pow(10,pba->mass_maj) ) > fabs(pba->full_thermo_lower_limit)){
        if ((pvecback_integration[pba->index_bi_a]*(1. + aux_fine_step_size )) < pba->a_today) {
          tau_end = tau_start + aux_fine_step_size / (pvecback_integration[pba->index_bi_a]*pvecback[pba->index_bg_H]);
        ...
        if ((pvecback_integration[pba->index_bi_a]*(1.+aux_step_size)) < pba->a_today) {
          tau_end = tau_start + aux_step_size / (pvecback_integration[pba->index_bi_a]*pvecback[pba->index_bg_H]);
          /* no possible segmentation fault here: non-zeroness of "a" has been checked in background_functions() */
        }
        else {
          tau_end = tau_start + (pba->a_today/pvecback_integration[pba->index_bi_a]-1.) / (pvecback_integration[pba->index_bi_a]*pvecback[pba->index_bg_H]);
...
int background_initial_conditions(
  ...
  /* majoron-neutrino -- start */
  if (pba->has_maj == _TRUE_){
    double Temp_gamma     = log(pba->T_cmb / pow(10,-14) * _kb_);             // class starting scale factor / redshift is fixed       
    double Neff_nu        = 3.044;
    double Temp_nu_int    = log( pow(_E_,Temp_gamma) * pow(4./11.,1./3.) * pow(Neff_nu/3.,1./4.) );         // this ratio is fixed to 1.40613   
    double Temp_maj       = log( pow(_E_,Temp_gamma) * pow(4./11.,1./3.) * pow(Neff_nu/3.,1./4.) * pow(pba->delta_neff_majoron * 2./(pba->gphi) ,1./4.));
    double mu_maj         = - pow(10.,pba->mu_maj_ini) * pow(_E_, Temp_gamma); 
    double mu_nu_int      = - pow(10.,pba->mu_nu_ini) * pow(_E_, Temp_nu_int);             
    pvecback_integration[pba->index_bi_Temp_maj]        = Temp_maj; 
    pvecback_integration[pba->index_bi_Temp_nu_int]     = Temp_nu_int;
    pvecback_integration[pba->index_bi_mu_maj]          = mu_maj;
    pvecback_integration[pba->index_bi_mu_nu_int]       = mu_nu_int;

    // rho_nu_int only for testing purpose
    double rho_nu_int = -_eV4_to_Mpc2_* pba->N_nu_maj_int *pba->gnu*3./pow(_PI_,2)*pow(pow(_E_,Temp_nu_int),4.)* 
                        polylog(4,-pow(_E_,mu_nu_int / pow(_E_,Temp_nu_int)));
    double rho_maj    = _eV4_to_Mpc2_ * pba->gphi* pow(pow(10,pba->mass_maj),2.) * pow(_E_,Temp_maj) * 
                        exp(mu_maj / pow(_E_,Temp_maj)) * 1./2. * 1./pow(_PI_,2.) * 
                        (pow(10,pba->mass_maj) * bessel(1,pow(10,pba->mass_maj) / pow(_E_,Temp_maj)) + 
                        3. * pow(_E_,Temp_maj) * bessel(2,pow(10,pba->mass_maj) / pow(_E_,Temp_maj)));


    printf("Infered analytical interacting neutrino density: %e.\n", pba->Omega0_nu_int*pow(pba->H0,2)/pow(a/pba->a_today,4));
    printf("Interacting neutrino density with massless FD thermodynamics: %e.\n", rho_nu_int);
    Omega_rad += pba->Omega0_nu_int;
    rho_rad   += rho_maj;
  }
  /* majoron-neutrino -- end */
...
int background_output_titles(struct background * pba,
  ...
  /* majoron-neutrino -- start */
  class_store_columntitle(titles,"T_maj [eV]", pba->has_maj);
  class_store_columntitle(titles,"T_nu_int [eV]", pba->has_maj);
  class_store_columntitle(titles,"mu_maj [eV]", pba->has_maj);
  class_store_columntitle(titles,"mu_nu_int [eV]", pba->has_maj);
  class_store_columntitle(titles,"rho_maj [class]", pba->has_maj);
  class_store_columntitle(titles,"p_maj [class]", pba->has_maj);
  class_store_columntitle(titles,"rho_nu_int [class]", pba->has_maj);
  /* majoron-neutrino -- end */
...
int background_output_data(
  ...
  /* majoron-neutrino -- start */
    class_store_double(dataptr, pow(_E_,pvecback[pba->index_bg_Temp_maj]),pba->has_maj,storeidx);
    class_store_double(dataptr, pow(_E_,pvecback[pba->index_bg_Temp_nu_int]),pba->has_maj,storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_mu_maj],pba->has_maj,storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_mu_nu_int],pba->has_maj,storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_rho_maj],pba->has_maj,storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_p_maj],pba->has_maj,storeidx);
    class_store_double(dataptr, pvecback[pba->index_bg_rho_nu_int],pba->has_maj,storeidx);
  /* majoron-neutrino -- end */
...
int background_derivs(
  ...
  /* majoron-neutrino -- start */
  double * pvecback, a, H, rho_M, Temp_maj, Temp_nu_int, mu_maj, mu_nu_int;
  /* majoron-neutrino -- end */
  ...
  /* majoron-neutrino -- start */
  // ODE
  double Hev, lambda, mass_maj, rho_maj, rho_maj_temp, rho_nu_temp, n_nu_temp, rho_ncdm_temp, Temp_gamma;
  if(pba->has_maj == _TRUE_){
    // converts 1eV to 1/Mpc
    double unit_corrector = 1./_hbar_ * 1./_sol_ * 1./_m_to_Mpc_;       
    // converts the class Hubble parameter 1/Mpc to eV
    Hev                   = H * _sol_ * _m_to_Mpc_ * _hbar_;            
    // change to linear scale for the majoron mass and lambda
    mass_maj             = pow(10,pba->mass_maj);
    // cut off for gamma_eff because we know that the system thermalizes for higher interaction rates
    if (pow(10.,pba->gamma_eff_maj) < pow(10.,3.)){
      lambda  = pow(pow(10.,pba->gamma_eff_maj)*mass_maj/1e3,1./2.) * 4.*pow(10.,-12.);
    }
    else{
      lambda  = pow(pow(10.,3.)*mass_maj/pow(10.,3.),1./2.) * 4.*pow(10.,-12.);
    }


    static int index_dummy_rand = -1;
    index_dummy_rand++;
    int index_dummy_module_rand = pow(10,6);
    // cut off for gamma eff if there is a largish majoron primordial population
    double gamma_delta_neff_cut = 0.005;
    if (pba->delta_neff_majoron > 0.05){
      if (pow(10.,pba->gamma_eff_maj) < pow(10.,3.) & pow(10.,pba->gamma_eff_maj) > gamma_delta_neff_cut){
        if (fmod(index_dummy_rand, index_dummy_module_rand) == 0){
          printf("\nNO CUT-OFF:%f\n", pba->gamma_eff_maj);
        }
        lambda  = pow(pow(10.,pba->gamma_eff_maj)*mass_maj/1e3,1./2.) * 4.*pow(10.,-12.);
      }
      else if (pow(10.,pba->gamma_eff_maj) >= pow(10.,3.)){
        if (fmod(index_dummy_rand, index_dummy_module_rand) == 0){
          printf("\nHIGH CUT-OFF:%f\n", pba->gamma_eff_maj);
        }
        lambda  = pow(pow(10.,3.)*mass_maj/pow(10.,3.),1./2.) * 4.*pow(10.,-12.);  // needs to be same number as cut-off
      }
      else{
        if (fmod(index_dummy_rand, index_dummy_module_rand) == 0){
          printf("\nLOW CUT-OFF:%f\n", pba->gamma_eff_maj);
        }
        lambda  = pow(gamma_delta_neff_cut*mass_maj/pow(10.,3.),1./2.) * 4.*pow(10.,-12.);   // needs to be same number as cut-off
      }
    }

    double gnu               = pba->gnu;
    double gphi              = pba->gphi;
    double N_nu_maj_int      = pba->N_nu_maj_int;
    // Temp_gamma is only for the file we may write below, in eV
    Temp_gamma      = pba->T_cmb / a *_kb_;
    // natural log scale for temperatures to avoid negative entries
    Temp_maj        = pow(_E_,y[pba->index_bi_Temp_maj]);     
    Temp_nu_int     = pow(_E_,y[pba->index_bi_Temp_nu_int]);   
    mu_maj          = y[pba->index_bi_mu_maj];
    mu_nu_int       = y[pba->index_bi_mu_nu_int];
    // some dimensionless densities
    rho_maj_temp    = (pvecback[pba->index_bg_rho_maj]/_eV4_to_Mpc2_) / pow(Temp_gamma,4.);   
    rho_nu_temp     = (pvecback[pba->index_bg_rho_nu_int])/_eV4_to_Mpc2_ / pow(Temp_gamma,4.);
    n_nu_temp       = -gnu*pow(Temp_nu_int,3.)/pow(_PI_,2.)*polylog(3,-exp(mu_nu_int/Temp_nu_int))/pow(Temp_gamma,4.);
    rho_ncdm_temp   = (pvecback[pba->index_bg_rho_ncdm1+0] /_eV4_to_Mpc2_)/ pow(Temp_gamma,4.);   
    // some checks for the ode solver
    static double decay_check;
    static int decay_check_points_aux = -1;
    decay_check_points_aux++;
    // these three numbers can depend on the step size defined in the .ini file -- "back_integration_stepsize"
    static int decay_check_points_num = pow(10,4);
    double full_thermo_lower_limit;
    if (gphi == 1){
      full_thermo_lower_limit    = pba->full_thermo_lower_limit;
    }
    else{
      full_thermo_lower_limit    = pba->full_thermo_lower_limit + 0.005;
    }

    double anem = (Temp_nu_int / mass_maj) / full_thermo_lower_limit;

    // start setting up the ode
    if (anem > 1){
      if ( (decay_check_points_aux % decay_check_points_num) == 0){
        decay_check = rho_maj_temp;
        if (pba->print_steps_maj == 1){
          printf("%e\n",Temp_nu_int / mass_maj);
        }
      }
      dy[pba->index_bi_Temp_maj]    = 1/Temp_maj * a * unit_corrector *
                                      (Temp_maj*(pow(_E_,mu_maj/Temp_maj)*Temp_maj*(pow(lambda,2)*pow(mass_maj,2)*N_nu_maj_int*
                                      pow(bessel(1,mass_maj/Temp_maj),2) + 
                                      3*pow(lambda,2)*mass_maj*N_nu_maj_int*Temp_maj*bessel(1,mass_maj/Temp_maj)*bessel(2,mass_maj/Temp_maj) - 
                                      (pow(lambda,2)*pow(mass_maj,2)*N_nu_maj_int + 48*Hev*_PI_*Temp_maj)*pow(bessel(2,mass_maj/Temp_maj),2)) + 
                                      pow(_E_,(2*mu_nu_int)/Temp_nu_int)*pow(lambda,2)*mass_maj*N_nu_maj_int*Temp_nu_int*
                                      (mass_maj*bessel(2,mass_maj/Temp_nu_int)*bessel(2,mass_maj/Temp_maj) - 
                                      bessel(1,mass_maj/Temp_nu_int)*(mass_maj*bessel(1,mass_maj/Temp_maj) + 3*Temp_maj*bessel(2,mass_maj/Temp_maj)))))/
                                      (16.*pow(_E_,mu_maj/Temp_maj)*_PI_*(-(pow(mass_maj,2)*pow(bessel(1,mass_maj/Temp_maj),2)) - 
                                      3*mass_maj*Temp_maj*bessel(1,mass_maj/Temp_maj)*bessel(2,mass_maj/Temp_maj) + 
                                      (pow(mass_maj,2) + 3*pow(Temp_maj,2))*pow(bessel(2,mass_maj/Temp_maj),2)));
      dy[pba->index_bi_Temp_nu_int] = 1/Temp_nu_int * a * unit_corrector * 
                                      (-(gphi*pow(lambda,2)*pow(mass_maj,4)*(pow(_E_,(2*mu_nu_int)/Temp_nu_int)*Temp_nu_int*bessel(2,mass_maj/Temp_nu_int) - 
                                      pow(_E_,mu_maj/Temp_maj)*Temp_maj*bessel(2,mass_maj/Temp_maj))*polylog(2,-pow(_E_,mu_nu_int/Temp_nu_int))) + 
                                      6*gphi*pow(lambda,2)*pow(mass_maj,3)*Temp_nu_int*
                                      (pow(_E_,(2*mu_nu_int)/Temp_nu_int)*Temp_nu_int*bessel(1,mass_maj/Temp_nu_int) - 
                                      pow(_E_,mu_maj/Temp_maj)*Temp_maj*bessel(1,mass_maj/Temp_maj))*polylog(3,-pow(_E_,mu_nu_int/Temp_nu_int)) - 
                                      576*Hev*_PI_*pow(Temp_nu_int,4)*pow(polylog(3,-pow(_E_,mu_nu_int/Temp_nu_int)),2) + 
                                      768*Hev*_PI_*pow(Temp_nu_int,4)*polylog(2,-pow(_E_,mu_nu_int/Temp_nu_int))*polylog(4,-pow(_E_,mu_nu_int/Temp_nu_int)))/
                                      (192.*_PI_*pow(Temp_nu_int,3)*(3*pow(polylog(3,-pow(_E_,mu_nu_int/Temp_nu_int)),2) - 
                                      4*polylog(2,-pow(_E_,mu_nu_int/Temp_nu_int))*polylog(4,-pow(_E_,mu_nu_int/Temp_nu_int))));
      dy[pba->index_bi_mu_maj]      = a * unit_corrector *
                                      (pow(_E_,mu_maj/Temp_maj)*pow(mass_maj,2)*(-48*Hev*_PI_ - pow(lambda,2)*N_nu_maj_int*(mu_maj - 3*Temp_maj))*Temp_maj*
                                      pow(bessel(1,mass_maj/Temp_maj),2) - pow(_E_,(2*mu_nu_int)/Temp_nu_int)*pow(lambda,2)*pow(mass_maj,2)*N_nu_maj_int*
                                      Temp_nu_int*(mu_maj - 3*Temp_maj)*bessel(2,mass_maj/Temp_nu_int)*bessel(2,mass_maj/Temp_maj) + 
                                      pow(_E_,mu_maj/Temp_maj)*Temp_maj*(pow(lambda,2)*pow(mass_maj,2)*N_nu_maj_int*(mu_maj - 3*Temp_maj) + 
                                      48*Hev*_PI_*(pow(mass_maj,2) + mu_maj*Temp_maj))*pow(bessel(2,mass_maj/Temp_maj),2) + 
                                      bessel(1,mass_maj/Temp_maj)*(pow(_E_,(2*mu_nu_int)/Temp_nu_int)*pow(lambda,2)*pow(mass_maj,3)*N_nu_maj_int*Temp_nu_int*
                                      bessel(2,mass_maj/Temp_nu_int) - 3*pow(_E_,mu_maj/Temp_maj)*mass_maj*
                                      (64*Hev*_PI_ + pow(lambda,2)*N_nu_maj_int*(mu_maj - 4*Temp_maj))*pow(Temp_maj,2)*bessel(2,mass_maj/Temp_maj))\
                                      + pow(_E_,(2*mu_nu_int)/Temp_nu_int)*pow(lambda,2)*mass_maj*N_nu_maj_int*Temp_nu_int*bessel(1,mass_maj/Temp_nu_int)*
                                      (mass_maj*(mu_maj - 3*Temp_maj)*bessel(1,mass_maj/Temp_maj) - 
                                      (pow(mass_maj,2) - 3*mu_maj*Temp_maj + 12*pow(Temp_maj,2))*bessel(2,mass_maj/Temp_maj)))/
                                      (16.*pow(_E_,mu_maj/Temp_maj)*_PI_*(pow(mass_maj,2)*pow(bessel(1,mass_maj/Temp_maj),2) + 
                                      3*mass_maj*Temp_maj*bessel(1,mass_maj/Temp_maj)*bessel(2,mass_maj/Temp_maj) - 
                                      (pow(mass_maj,2) + 3*pow(Temp_maj,2))*pow(bessel(2,mass_maj/Temp_maj),2)));
      dy[pba->index_bi_mu_nu_int]   = a * unit_corrector *
                                      -0.005208333333333333*(gphi*pow(lambda,2)*pow(mass_maj,4)*mu_nu_int*
                                      (pow(_E_,(2*mu_nu_int)/Temp_nu_int)*Temp_nu_int*bessel(2,mass_maj/Temp_nu_int) - 
                                      pow(_E_,mu_maj/Temp_maj)*Temp_maj*bessel(2,mass_maj/Temp_maj))*polylog(2,-pow(_E_,mu_nu_int/Temp_nu_int)) - 
                                      3*gphi*pow(lambda,2)*pow(mass_maj,3)*Temp_nu_int*
                                      (pow(_E_,(2*mu_nu_int)/Temp_nu_int)*Temp_nu_int*(2*mu_nu_int*bessel(1,mass_maj/Temp_nu_int) + mass_maj*bessel(2,mass_maj/Temp_nu_int)) - 
                                      pow(_E_,mu_maj/Temp_maj)*Temp_maj*(2*mu_nu_int*bessel(1,mass_maj/Temp_maj) + mass_maj*bessel(2,mass_maj/Temp_maj)))*
                                      polylog(3,-pow(_E_,mu_nu_int/Temp_nu_int)) + 576*Hev*mu_nu_int*_PI_*pow(Temp_nu_int,4)*
                                      pow(polylog(3,-pow(_E_,mu_nu_int/Temp_nu_int)),2) + 
                                      24*gphi*pow(lambda,2)*pow(mass_maj,3)*pow(Temp_nu_int,2)*
                                      (pow(_E_,(2*mu_nu_int)/Temp_nu_int)*Temp_nu_int*bessel(1,mass_maj/Temp_nu_int) - 
                                      pow(_E_,mu_maj/Temp_maj)*Temp_maj*bessel(1,mass_maj/Temp_maj))*polylog(4,-pow(_E_,mu_nu_int/Temp_nu_int)) - 
                                      768*Hev*mu_nu_int*_PI_*pow(Temp_nu_int,4)*polylog(2,-pow(_E_,mu_nu_int/Temp_nu_int))*polylog(4,-pow(_E_,mu_nu_int/Temp_nu_int)))/
                                      (_PI_*pow(Temp_nu_int,4)*(3*pow(polylog(3,-pow(_E_,mu_nu_int/Temp_nu_int)),2) - 
                                      4*polylog(2,-pow(_E_,mu_nu_int/Temp_nu_int))*polylog(4,-pow(_E_,mu_nu_int/Temp_nu_int))));
    }
    else{
      //(non-)rel limiting cases..
      if (Temp_maj / mass_maj > 1){
        dy[pba->index_bi_Temp_maj]        = - 1. * a * unit_corrector * Hev;
        dy[pba->index_bi_Temp_nu_int]     = - a * unit_corrector * Hev;
        dy[pba->index_bi_mu_maj]          = - 1. * a * unit_corrector * Hev * mu_maj;
        dy[pba->index_bi_mu_nu_int]       = - a * unit_corrector * Hev * mu_nu_int;
      }
      else{
        dy[pba->index_bi_Temp_maj]        = - 2. * a * unit_corrector * Hev;
        dy[pba->index_bi_Temp_nu_int]     = - a * unit_corrector * Hev;
        dy[pba->index_bi_mu_maj]          = - 2. * a * unit_corrector * Hev * (mu_maj - 0);
        dy[pba->index_bi_mu_nu_int]       = - a * unit_corrector * Hev * mu_nu_int;
      }
    }
  }
  ...
  if (pba->wcbf > 0){
    if (pba->has_maj == _TRUE_){
      static int index_dummy_wcbf_header = -1;
      index_dummy_wcbf_header++;
      if (index_dummy_wcbf_header == 0){
        FILE *dummy_file_pba;
        char path[200];
        getcwd(path, 200);
        strcat(path, "/output/dummy_file_pba.dat");
        dummy_file_pba = fopen(path,"a");
        fprintf(dummy_file_pba, "# initial conditions: N  gphi  m gamma_eff DeltaN_eff \n");
        fprintf(dummy_file_pba, "%e\t %e\t %e\t %e\t %e\n",pba->N_nu_maj_int, pba->gphi, log10(mass_maj), pba->gamma_eff_maj, pba->delta_neff_majoron);
        fclose(dummy_file_pba); 
      }

      static int index_dummy_wcbf = -1;
      index_dummy_wcbf++;
      int index_dummy_module_wcbf = pow(10,2);
      if (fmod(index_dummy_wcbf, index_dummy_module_wcbf) == 0){
        FILE *dummy_file_pba;
        char path[200];
        getcwd(path, 200);
        strcat(path, "/output/dummy_file_pba.dat");
        dummy_file_pba = fopen(path,"a");
        if (pba->N_nu_maj_int == 3){
          fprintf(dummy_file_pba,"%e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\n", log10(mass_maj), a, Temp_nu_int, rho_maj_temp, rho_nu_temp , mu_maj/Temp_gamma, mu_nu_int/Temp_gamma, Hev, pvecback[pba->index_bg_p_maj], Temp_gamma, mu_maj/Temp_maj, mu_nu_int/Temp_nu_int, Temp_maj, n_nu_temp);
        }
        else{
          fprintf(dummy_file_pba,"%e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\t %e\n", log10(mass_maj), a, Temp_nu_int, rho_maj_temp, rho_nu_temp , mu_maj/Temp_gamma, mu_nu_int/Temp_gamma, Hev, pvecback[pba->index_bg_p_maj], Temp_gamma, mu_maj/Temp_maj, mu_nu_int/Temp_nu_int, Temp_maj, n_nu_temp, rho_ncdm_temp);
        }
        fclose(dummy_file_pba);  
      }
    }
...
int background_output_budget(
  ...
  /* majoron-neutrino -- start */
    if(pba->has_maj){
      _class_print_species_("Interacting Neutrino relics",nu_int);
      budget_radiation+=pba->Omega0_nu_int;
      _class_print_species_("Majoron relic",maj);
      budget_radiation+=0.0;
    }
  /* majoron-neutrino -- end */
