...
/* majoron-neutrino -- start */
double fun_pert(double x){
  if( x < 50 && x > 0){
    return pow(x,5)*1./2.*pow(_E_,-x)*(-1. + x - pow(_E_,x)*(x*x - 2.) * gamma_inc(0,x) );
  }
  else{
    if(x < 0){
      printf(" >> error in gamma function in pert. Return 0. <<");
      return 0.;
    }
    else{
      return 0.0;
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    if (pba->has_maj == _TRUE_) {
      // neutrino
      rho_r   += ppw->pvecback[pba->index_bg_rho_nu_int];
      rho_nu  += ppw->pvecback[pba->index_bg_rho_nu_int];
      // majoron
      rho_r   += ppw->pvecback[pba->index_bg_rho_maj];
      rho_nu  += ppw->pvecback[pba->index_bg_rho_maj];
    }
    /* majoron-neutrino -- end */
...
int perturb_einstein(
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      /* majoron-neutrino -- start */
      // synchronous gauge implementation
      if (pba->has_maj == _TRUE_) { 
        ppw->rho_plus_p_theta += 4./3.*(ppw->pvecback[pba->index_bg_rho_nu_int])*ppw->rsa_theta_ur;
        ppw->rho_plus_p_theta += (ppw->pvecback[pba->index_bg_rho_maj] + ppw->pvecback[pba->index_bg_p_maj])*ppw->rsa_theta_ur;
      }
      /* majoron-neutrino -- end */
...
int perturb_total_stress_energy(
  ...
  if (_scalars_) {
    ...
    /* majoron-neutrino -- start */
    if (pba->has_maj == _TRUE_) {
      ppw->delta_rho          += (ppw->pvecback[pba->index_bg_rho_nu_int] )*delta_ur;
      ppw->rho_plus_p_theta   += 4./3.*(ppw->pvecback[pba->index_bg_rho_nu_int] )*theta_ur;
      ppw->rho_plus_p_shear   += 4./3.*(ppw->pvecback[pba->index_bg_rho_nu_int] )*shear_ur;
      ppw->delta_p            += 1./3.*(ppw->pvecback[pba->index_bg_rho_nu_int] )*delta_ur;
      ppw->rho_plus_p_tot     += 4./3. * (ppw->pvecback[pba->index_bg_rho_nu_int] );
      
      ppw->delta_rho          += ppw->pvecback[pba->index_bg_rho_maj]*delta_ur;
      ppw->rho_plus_p_theta   += (ppw->pvecback[pba->index_bg_rho_maj] + ppw->pvecback[pba->index_bg_p_maj])*theta_ur;
      ppw->rho_plus_p_shear   += (ppw->pvecback[pba->index_bg_rho_maj] + ppw->pvecback[pba->index_bg_p_maj])*shear_ur;
      ppw->delta_p            += ppw->pvecback[pba->index_bg_p_maj]*delta_ur;
      ppw->rho_plus_p_tot     += ppw->pvecback[pba->index_bg_rho_maj] + ppw->pvecback[pba->index_bg_p_maj];
    }
    /* majoron-neutrino -- end */
  ...
  if (_tensors_) {
    ...
    if (ppt->evolve_tensor_ur == _TRUE_){
      ...
      if (ppt->tensor_method == tm_exact)
        ...
        /* majoron-neutrino -- start */
        if(pba->has_maj == _TRUE_){
          rho_relativistic += ppw->pvecback[pba->index_bg_rho_nu_int];
          rho_relativistic += ppw->pvecback[pba->index_bg_rho_maj];
        }
        /* majoron-neutrino -- end */
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    /* majoron-neutrino -- start */
    // code all the interacting neutrino - majoron perturbations in the ur limit
    if (pba->has_ur == _TRUE_) {

      // define majoron related variables
      double lambda;
      double Gamma_nu_int;
      double Temp_nu_int;
      double Temp_maj;
      ...
      //get relevant variables in short hand notation
      if (pba->has_maj == _TRUE_){
        rho_nu        = pvecback[pba->index_bg_rho_nu_int];
        p_nu          = rho_nu/3.;
        rho_maj       = pvecback[pba->index_bg_rho_maj];
        p_maj         = pvecback[pba->index_bg_p_maj];
        // w_maj         = 1./3.;
        w_maj         = (p_nu+p_maj)/(rho_nu+rho_maj);
        Temp_nu_int   = pow(_E_,pvecback[pba->index_bg_Temp_nu_int]);  
        Temp_maj      = pow(_E_,pvecback[pba->index_bg_Temp_maj]);
        mu_nu_int     = pvecback[pba->index_bg_mu_nu_int];
        mu_maj        = pvecback[pba->index_bg_mu_maj];
      }
      ...
      if (ppw->approx[ppw->index_ap_rsa] == (int)rsa_off) {
        ...
        if  (pba->has_maj == _TRUE_){
          ...
          // calculate non-standard sound speed 
          double p_tot_h (){
            double h = pow(10,-5);
            class_call(background_at_tau(pba,
              tau - h,
            ...
            return pvecback[pba->index_bg_rho_nu_int]/3.0 + pvecback[pba->index_bg_p_maj];
          }
          double rho_tot_h (){
            double h = pow(10,-5);
            class_call(background_at_tau(pba,
            ...
            return pvecback[pba->index_bg_rho_nu_int] + pvecback[pba->index_bg_rho_maj];
          }
          cs2_maj = (p_tot_h() - (p_nu + p_maj))/ (rho_tot_h() - (rho_maj + rho_nu));
          ...
          dy[pv->index_pt_delta_ur] =
            // modified ur perturbations
            -(1 + w_maj)*(y[pv->index_pt_theta_ur] + metric_continuity)
            -3.*(cs2_maj-w_maj)*a_prime_over_a*y[pv->index_pt_delta_ur];

          /** - -----> ur velocity */
          dy[pv->index_pt_theta_ur] =
            // modified ur perturbations
            -(1.-3.*cs2_maj)*a_prime_over_a*y[pv->index_pt_theta_ur]
            +cs2_maj*k2/(1.+w_maj)*y[pv->index_pt_delta_ur]
            -k2*s2_squared*y[pv->index_pt_shear_ur]
            +metric_euler;

        }
        else{
          //these are the standard public class ur functions
          /** - -----> ur density */
          dy[pv->index_pt_delta_ur] =
            // standard term
        ...
        if(ppw->approx[ppw->index_ap_ufa] == (int)ufa_off) {
          ...
          // perturbation from interactions -- start
          if(pba->has_maj == _TRUE_){
            

            lambda    = pow(pow(10,pba->gamma_eff_maj)*pow(10,pba->mass_maj)/pow(10,3.),1./2.) * 4.*pow(10.,-12.);

            double Gamma_nu_int_new  = pow(lambda,2.)/96./_PI_*1./(3.*1.20206)*pba->gphi/pba->gnu*fun_pert(pow(10,pba->mass_maj)/Temp_nu_int) * pow(_hbar_*_sol_,-1.) * pow(_m_to_Mpc_,-1.);   //  eV to 1/Mpc

            double alpha2 = 1./2.;
            double alpha3 = 27./4.;

            dy[pv->index_pt_shear_ur] -=  alpha2 * a * Gamma_nu_int_new * y[pv->index_pt_shear_ur];

            l = 3;
            dy[pv->index_pt_l3_ur]    -=  alpha3 * a * Gamma_nu_int_new * y[pv->index_pt_l3_ur];

            /** - -----> exact ur l>3 */
            for (l = 4; l < pv->l_max_ur; l++) {
              dy[pv->index_pt_delta_ur+l] -= 1./32.*(3.*pow(l,4.) + 2.*pow(l,3.) - 11.*pow(l,2.) + 6.*l)*a*Gamma_nu_int_new * y[pv->index_pt_delta_ur+l];
            }

            // costum file
            if (ppt->wcpf > 0){
              static int index_dummy_wcpf = -1;
              index_dummy_wcpf++;
              int index_dummy_module_wcpf = pow(10,2);
              if (fmod(index_dummy_wcpf, index_dummy_module_wcpf) == 0){
                FILE *dummy_file_ppt;
                char path[200];
                getcwd(path, 200);
                strcat(path, "/output/dummy_file_ppt.dat");
                dummy_file_ppt = fopen(path,"a");
                fprintf(dummy_file_ppt,"%e\t %e\t %e\t %e\t %e\n", a, Temp_nu_int/pow(10.,pba->mass_maj), w_maj, cs2_maj, alpha2 * Gamma_nu_int_new/pvecback[pba->index_bg_H] );
                fclose(dummy_file_ppt);  
              }
            }
          }
          // perturbations from interactions -- end
...
int perturb_rsa_delta_and_theta(
  ...
  /* majoron-neutrino -- start */
  if (pba->has_maj == _TRUE_) {
    ppw->delta_rho        += ppw->pvecback[pba->index_bg_rho_nu_int]*ppw->rsa_delta_ur;
    ppw->delta_p          += 1./3.*(ppw->pvecback[pba->index_bg_rho_nu_int])*ppw->rsa_delta_ur;
    ppw->rho_plus_p_theta += 4./3.*(ppw->pvecback[pba->index_bg_rho_nu_int])*ppw->rsa_theta_ur;

    ppw->delta_rho        += ppw->pvecback[pba->index_bg_rho_maj]*ppw->rsa_delta_ur;
    ppw->delta_p          += 1./3.*ppw->pvecback[pba->index_bg_rho_maj]*ppw->rsa_delta_ur;
    ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_maj]*ppw->rsa_theta_ur;
  }
  /* majoron-neutrino -- end */
