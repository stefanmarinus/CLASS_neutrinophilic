...
int input_read_parameters(struct file_content * pfc,
  ...
  // print initial conditions for information
  printf("INITIAL CONDITIONS:\n");
  printf("N_int     = %f\n", pba->N_nu_maj_int);
  printf("dneff     = %e\n", pba->delta_neff_majoron);
  printf("gamma     = %f\n", pba->gamma_eff_maj);
  printf("lambda    = %f\n", log10( sqrt( pow(10,pba->gamma_eff_maj) * pow(10,pba->mass_maj)/pow(10,3.)) * 4.*pow(10,-12.)));
  printf("mass      = %f\n", pba->mass_maj);
  printf("H0        = %f\n", 100*pba->h);
  printf("Omega_b   = %f\n", pba->Omega0_b);
  printf("Omega_cdm = %f\n", pba->Omega0_cdm);
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  int fileentries;
  /* majoron - neutrino -- start */
  // change N_ncdm=0 to just N_ncdm
  int N_ncdm, n, entries_read;
  /* majoron - neutrino -- end */
  double rho_ncdm;
  double scf_lambda;
  double fnu_factor;
  ...
  /* majorin - neutrino */
  //set the _ur contribution to 0, unless the .ini file specifies N_ur, N_eff, Omega_ur or omega_ur
  if (class_none_of_three(flag1,flag2,flag3)) {
    pba->Omega0_ur = 0*3.044*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  }
  else {
    if (flag1 == _TRUE_) {
  ...
  /* majoron - neutrino -- start*/
  // put here the majoron related stuff right before the ncdm class stuff
  class_read_double("mu_maj", pba->mu_maj_ini);
  class_read_double("mu_nu", pba->mu_nu_ini);
  class_read_double("mass_maj", pba->mass_maj);
  class_read_double("gnu", pba->gnu);
  class_read_double("gphi", pba->gphi);
  class_read_double("delta_neff_majoron", pba->delta_neff_majoron);
  class_read_double("decay_ratio", pba->decay_ratio);
  class_read_double("full_thermo_lower_limit", pba->full_thermo_lower_limit);
  //check for lambda or gamma, only one of them ok
  class_call(parser_read_double(pfc,"gamma_eff_maj",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"lambda_maj",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of gamma_eff_maj or lambda_maj, choose one");
  if (flag1 == _TRUE_){
    pba->gamma_eff_maj = param1;
  }
  if (flag2 == _TRUE_){
    pba->gamma_eff_maj = log10( pow( pow(10,param2)/(4.*pow(10.,-12.)),2.)*pow(10.,3.)/pow(10,pba->mass_maj) );
  }
  class_read_double("print_steps_maj", pba->print_steps_maj);
  class_read_double("fine_steps_maj", pba->fine_steps_maj);
  class_read_double("wcbf",pba->wcbf);
  class_read_double("wcpf",ppt->wcpf);

  class_read_double("N_nu_maj_int", pba->N_nu_maj_int);

  // cut off for (very) small gamma_eff. Change to free evolution because system does not thermalize
  if (pba->delta_neff_majoron < 0.01 && pba->delta_neff_majoron < 0.01){
    if (pba->gamma_eff_maj < -3.01){
      pba->N_nu_maj_int = 0;
    }
  }
  ...
  if (ppt->wcpf > 0){
    char path2[200];
    getcwd(path2, 200);
    strcat(path2, "/output/dummy_file_ppt.dat");
    remove(path2);
  } 


  if (pba->N_nu_maj_int == 3){
    pba->Omega0_nu_int  = 3.044*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  }
  else{
    pba->Omega0_nu_int  = pba->N_nu_maj_int*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  }
  pba->Omega0_maj     = 0.0;
  /* majoron - neutrino -- end*/
  ...
  /* majoron - neutrio -- start */
  // comment the class_read_int line.
  ...
  if (N_ncdm > 0){
    ...
    for(n=0; n<N_ncdm; n++){
      ...
      if ((pba->Omega0_ncdm[n]==0.0) && (pba->m_ncdm_in_eV[n]==0.0)) {
        /* majoron - neutrio -- start */
        // change 1e-5 to 1e-10
        pba->m_ncdm_in_eV[n]=1.e-10;
        /* majoron - neutrio -- end */
      }
    }
    ...
    for (n=0; n < N_ncdm; n++){
      if (pba->m_ncdm_in_eV[n] != 0.0){
        ...
        class_call(background_ncdm_momenta(pba->q_ncdm_bg[n],
                   ...
                   errmsg);
        /* majoron - neutrino -- start */
        // these numbers are chosen to give N_eff = 3.044 arising from the Neutrinos
        if (pba->N_nu_maj_int == 0){
          pba->Omega0_ncdm[n] = 3.77226*pow(10,-5);
        }
        else if (pba->N_nu_maj_int == 1){
          pba->Omega0_ncdm[n] = 2.53325*pow(10,-5);
        }
        else if (pba->N_nu_maj_int == 2){
          pba->Omega0_ncdm[n] = 1.29400*pow(10,-5);
        }
        else{
          pba->Omega0_ncdm[n] = 0;
        }
        ...
        if (pba->Omega0_ncdm[n] == 0.0){
          ...
          if (pba->N_nu_maj_int == 0){
            pba->deg_ncdm[n] = 3.00441;
          }
          else if (pba->N_nu_maj_int == 1){
            pba->deg_ncdm[n] = 2.0176;
          }
          else if (pba->N_nu_maj_int == 2){
            pba->deg_ncdm[n] = 1.030666666;
          }
          else{
            pba->deg_ncdm[n] = 0;
          }
  ...
  /* majoron - neutrino -- start */
  Omega_tot += pba->Omega0_nu_int;
  Omega_tot += pba->Omega0_maj;
  /* majoron - neutrino -- end */
  /* Step 1 */
  if (flag1 == _TRUE_){
...
int input_default_params(struct background *pba,
  ...
  /* neutrino - majoron -- start*/
  // turn off standard ur contribution
  pba->Omega0_ur = 0.0 * 3.044*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
  /* neutrino - majoron -- end*/
  ...
  /* neutrino - majoron -- start*/
  pba->N_nu_maj_int             = 3.0;
  pba->wcbf                     = 0;
  pba->print_steps_maj          = 0.;
  pba->delta_neff_majoron       = 0.00000000001;   // can not be 0 
  pba->mu_maj_ini               = -10.;
  pba->mu_nu_ini                = -10.; 
  pba->mass_maj                 = 0; 
  pba->gamma_eff_maj            = 1; 
  pba->gnu                      = 2.;
  pba->gphi                     = 1.;
  pba->decay_ratio              = 0.01;
  pba->full_thermo_lower_limit  = 0.065;
  pba->fine_steps_maj           = 2.*pow(10,-5);
  /* neutrino - majoron -- end*/
  ...
  pba->Omega0_scf = 0.;
  /* majoron-neutrino -- start*/
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_nu_int-pba->Omega0_maj-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr+pba->Omega0_idr+pba->Omega0_idm_dr;
  /* majoron-neutrino -- end*/
  /** 8.a) Omega fluid */
  /** 8.a.1) PPF approximation */
  pba->use_ppf = _TRUE_;
